...
#-------------------------------------------------------------------------------
system = Molecule() 
system.load_PDB_to_system      (filename = os.path.join(PEPDICE_EXAMPLES , 'data/1gab_files/2bjb.pdb'   )   )   
...
'''     
    ...
    for i in range (0,len(system.residues)):
        ...
        if log:
            print i, "%s  %15.5f  %15.5f  %15.5f" %(system.residues[i].name , phi_final_angle, psi_final_angle, ome_final_angle)
...
torsions  = test_computeTorsions   (system = system, log = True)
pprint(torsions)
